...
function Toggle({ value, onChange }: { value: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      className={`relative flex-shrink-0 w-12 h-6 rounded-full transition-colors ${value ? 'bg-emerald-500' : 'bg-slate-700'}`}
    >
  ...
  return (
    <div className="p-8 max-w-5xl">
      ...
      <Section title="Privacy" icon={ShieldCheck}>
        <div className="flex items-start gap-3 bg-emerald-950/30 border border-emerald-900/40 rounded-xl p-3">
          <ShieldCheck size={18} className="text-emerald-500 flex-shrink-0 mt-0.5" />
          <div>
            <p className="text-sm text-emerald-300 font-medium">100% on-device processing</p>
      ...
      <Section title="Data" icon={Trash2}>
        {showConfirm ? (
          <div className="space-y-3">
            <p className="text-sm text-red-300">This will erase your entire history, reset your streak, and cannot be undone.</p>
            <div className="flex gap-2">
              <button
                onClick={() => { clearAllData(); setShowConfirm(false); }}
                className="flex-1 bg-red-900 hover:bg-red-800 border border-red-700 rounded-xl py-2 text-sm text-red-200 transition-colors"
              >
                Yes, clear everything
              </button>
              <button
                onClick={() => setShowConfirm(false)}
                className="flex-1 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-xl py-2 text-sm text-slate-300 transition-colors"
              >
                Cancel
              </button>
            </div>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 text-red-400 hover:text-red-300 bg-red-950/30 hover:bg-red-950/50 border border-red-900/40 rounded-xl py-2.5 text-sm transition-colors"
          >
            <Trash2 size={14} />
            Clear all data
          </button>
