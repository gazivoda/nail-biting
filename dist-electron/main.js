import { BrowserWindow, Menu, Notification, Tray, app, ipcMain, nativeImage, session, shell, systemPreferences } from "electron";
...
function loadEnv() {
	...
	for (const envPath of candidates) if (existsSync(envPath)) {
...
var RENDERER_FILE = path.join(__dirname, "..", "dist", "index.html");
var API_BASE = isDev ? `http://localhost:${process.env.PORT || 3e3}` : process.env.API_BASE_URL || "http://localhost:3000";
var SERVER_PORT = 3e3;
var PROTOCOL = "nailhabit";
var mainWindow = null;
var tray = null;
...
function handleDeepLink(url) {
	...
	try {
		...
		if (token) deliverTokenToRenderer(token);
			...
			const inject = () => {
				mainWindow.webContents.executeJavaScript(`window.dispatchEvent(new CustomEvent('electron-auth', { detail: { error: ${JSON.stringify(error)} } }))`).catch((err) => console.error("[main] executeJavaScript failed:", err));
			};
			if (mainWindow.webContents.isLoading()) mainWindow.webContents.once("did-finish-load", inject);
			else inject();
		}
...
async function createWindow() {
	if (process.platform === "darwin") {
		if (systemPreferences.getMediaAccessStatus("camera") !== "granted") await systemPreferences.askForMediaAccess("camera");
	}
	if (!isDev) session.defaultSession.webRequest.onBeforeRequest({ urls: ["file://*"] }, (details, callback) => {
		const url = new URL(details.url);
		if (url.pathname.startsWith("/api/")) callback({ redirectURL: `${API_BASE}${url.pathname}${url.search}` });
		else callback({});
	});
	...
	} else await mainWindow.loadFile(RENDERER_FILE);
	...
	mainWindow.webContents.setWindowOpenHandler(({ url }) => {
		if (!url.startsWith("http://localhost") && !url.startsWith("file://")) shell.openExternal(url);
		return { action: "deny" };
	});
...
function createTray() {
	const iconPath = isDev ? path.join(__dirname, "..", "public", "icons", "icon-32x32.png") : path.join(__dirname, "..", "dist", "icons", "icon-32x32.png");
...
function setupIPC() {
	...
	ipcMain.handle("open-google-auth", async () => {
		try {
			const googleUrl = (await fetch(`${API_BASE}/api/auth/google`, { redirect: "manual" })).headers.get("location");
			if (googleUrl) shell.openExternal(googleUrl);
		} catch (err) {
